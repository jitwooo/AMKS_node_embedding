...
from model.utils import compute_laplacian_eigenxx, compute_wks, get_padded_neighbor_vec
import numpy as np
...
class WKS:
    ...
    def compute_emb_vec(self):
        e, u, reinedex_dict = compute_laplacian_eigenxx(self._g)
        self._reindex_dict = reinedex_dict
        self._embedding_vec = compute_wks(e, u, self._sigma, self._step)
...
class MultiHopWKS(WKS):
    def __init__(self) -> None:
        super().__init__()
        self._maxhop = None
        self.model_name = 'MultiHopWKS'

    def set_maxhop(self, maxhop):
        self._maxhop = maxhop  # whz理解：即周正雄算法3-2中的 参数L

    def compute_spec_hop_wks(self, wks: np.ndarray, neighbors_index: np.ndarray):
        gathered_wks = wks[
            neighbors_index]  # whz理解：取出该范围邻居的嵌入向量表示，并求和，wks(2813,30),neighbors_index(2812,x),gathered_wks(2812,x,30)
        combined_wks = np.sum(gathered_wks,
                              axis=1)  # whz理解：按列x相加，三维矩阵变成二维矩阵,combined_wks(2812,30)，即把所有该层同心圆的邻居嵌入求和，作为一个表示
        return combined_wks

    def compute_multihop_wks(self):
        # base_logger.info("start multi-how comput now")
        wks = self._embedding_vec
        padded_wks = np.vstack([wks, np.zeros((1, wks.shape[1]))])  # np.vstack():在竖直方向上堆叠,相当于多了一行 全是0，即第2812行，用来代表没有邻居
        ...
        for hop in range(self._maxhop + 1):
            results[hop] = self.compute_spec_hop_wks(padded_wks, padded_neighbor_vec[hop])
